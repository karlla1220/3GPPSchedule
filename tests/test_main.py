...
def no_live_portal_lookup():
    with patch("working_groups.ran1.pipeline.lookup_timezone_reference", return_value=None):
        yield


def _portal_reference():
    return {'type': 'portal', 'id': 60713, 'timezone': 'Europe/Amsterdam',
            'starts_on': '2026-08-24', 'ends_on': '2026-08-28',
            'starts_at': '2026-08-24T09:00:00+02:00', 'ends_at': '2026-08-28T17:30:00+02:00',
            'start_timezone': '(GMT+02.00) Brussels, Copenhagen, Madrid, Paris',
            'end_timezone': '(GMT+02.00) Brussels, Copenhagen, Madrid, Paris',
            'location': 'Maastricht', 'country': 'NL'}


@pytest.mark.parametrize('offline', [False, True])
def test_portal_timezone_bypasses_document_location_and_llm(tmp_path, offline):
    from working_groups.ran1.models import ScheduleSource
    ...
    previous = {'meeting_id': 'ran1#126', 'timezone_status': 'resolved',
                'timezone': ref['timezone'], 'timezone_ref': ref}
    with MainExtraFilesWiringTests()._enter_common([], no_download=offline) as stack:
        stack.enter_context(patch.object(main_module, 'load_schedule_state', return_value=previous if offline else {}))
        ...
        lookup = stack.enter_context(patch.object(main_module, 'lookup_timezone_reference', return_value=ref))
        chair = stack.enter_context(patch.object(main_module, 'get_latest_chair_notes_info'))
        download = stack.enter_context(patch.object(main_module, 'download_latest_chair_notes'))
