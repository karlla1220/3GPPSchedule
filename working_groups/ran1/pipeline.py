...
Config file:
    config.json (working_groups/ran1/, optional) — see config.py for schema.
"""
...
from .models import DAY_ORDER, DaySchedule, Schedule, ScheduleSource, ran1_timeline
...
def _agenda_state_from_info(
    ...
    """Build the agenda block stored in docs/ran1/.schedule_state.json."""
...
def main():
    ...
    argparser.add_argument(
        "--output",
        type=str,
        default="docs/ran1/index.html",
        help="Output HTML file path (default: docs/ran1/index.html)",
    )
    argparser.add_argument(
        "--rebuild-slots",
        action="store_true",
        help=(
            "Wipe per-slot incremental state cache (docs/ran1/slot_state/) and "
            "rebuild from raw sources. Use when carry-forward errors have "
            "accumulated. Operators can also delete individual files from "
            "docs/ran1/slot_state/ via the GitHub web UI to force a cold "
    ...
    save_html(schedule, args.output)


def build_schedule(args) -> Schedule:
    """Run RAN1 discovery, parsing and merging; return the display schedule."""
    if args.rebuild_slots:
        from .slot_state import clear_all_slot_states

        n = clear_all_slot_states()
        print(
            f"Cleared {n} slot state file(s) from docs/ran1/slot_state/; "
    ...
    if args.local:
        ...
        if docx_path is None:
            ...
            if docx_path is None:
                print("Error: No schedule files found locally in downloads/ran1/Chair_notes/")
                sys.exit(1)
        ...
        vice_chair_paths = find_local_vice_chair_schedules(
            meeting_id=_extract_meeting_id(docx_path.name)
        )
        if vice_chair_paths:
        ...
        # downloads/ran1/extra_files/.  Schedule entries become local
        ...
        try:
            ...
            if sources:
                raise RuntimeError("Could not download all selected RAN1 sources") from e
    ...
    if agenda_urls and not args.no_download:
        ...
        if agenda_info is not None:
            agenda_candidate_ref = _agenda_state_from_info(agenda_info, None)
            ...
            agenda_path = download_latest_agenda(
                ...
                force=(
                    ...
                    and any((prev_state.get("agenda") or {}).get(k) != v
                            for k, v in agenda_candidate_ref.items())
                ),
            )
        else:
    ...
    # Prefer Portal metadata for this exact meeting. Reuse a resolved Portal
    # reference offline or during an outage; never borrow another meeting's TZ.
    portal_ref = (lookup_timezone_reference("ran1", current_meeting_id)
                  if not args.no_download else None)
    if (portal_ref is None and prev_state.get("meeting_id") == current_meeting_id
            and prev_state.get("timezone_status") == "resolved"
    ...
    if portal_ref is not None:
        ...
        timezone_ref = portal_ref
        print(f"\nPortal timezone for {current_meeting_id}: {meeting_tz}")
    else:
    ...
    if sources is not None:
        ...
        meeting_source = (
            ...
            and selected_main_source.origin == "local"
            else "remote"
        )
        save_schedule_state(
            [s for s in sources if s.origin == "remote"],
            meeting_id=current_meeting_id,
    ...
    schedule = Schedule(
        ...
        starts_at=portal_ref.get("starts_at") if portal_ref else None,
        ends_at=portal_ref.get("ends_at") if portal_ref else None,
    )
