...
State is persisted in docs/ran1/.schedule_state.json (committed to the repo
...
from shared.portal_meetings import lookup_timezone_reference
...
def check_updates(*, staging_dir=None) -> CheckResult:
    """Check RAN1 sources without committing their new state."""
    errors = []
    extra_files_artifact_ready = False
    staging_dir = staging_dir if staging_dir is not None else EXTRA_FILES_TRANSFER_DIR
    ...
    # A historical local build is only a hint. Lock while an authoritative
    # local source is still present, so removing it lets the next meeting win.
    locked_meeting_id = local_meeting_hint
    if locked_meeting_id is None and cfg.get("extra_files"):
        external = load_external_files_state()
        if external.get("config") == _external_config_fingerprint(cfg["extra_files"]):
            ids = set()
            for entry in cfg["extra_files"]:
                if entry.get("type") != "schedule" or not entry.get("is_main"):
                    continue
                filename = external.get("files", {}).get(entry["url"], {}).get("filename")
                if filename and (EXTRA_FILES_DIR / filename).is_file():
                    meeting = _extract_meeting_id(filename)
                    if meeting:
                        ids.add(meeting)
            regular = [meeting for meeting in ids if _meeting_rank(meeting) is not None]
            if regular:
                locked_meeting_id = max(regular, key=_meeting_rank)
            elif len(ids) == 1:
                locked_meeting_id = next(iter(ids))
            preferred_meeting_id = locked_meeting_id or preferred_meeting_id
    ...
    try:
        ...
        errors.append(f"FTP check failed: {e}")
    ...
    if portal_ref is not None or (
        ...
        if agenda_ref is not None and any(
            ...
            for key, value in agenda_ref.items() if key != "type"
        ):
            changed = True
            print("Agenda description source changed")
    if portal_ref is not None:
        if portal_ref != cached_timezone_ref or state.get("timezone_status") != "resolved":
            changed = True
            print(f"Portal timezone metadata changed: {portal_ref!r}")
    elif isinstance(cached_timezone_ref, dict) and cached_timezone_ref.get("type") == "portal":
        # A transient Portal failure must not downgrade a resolved API source.
        pass
    else:
    ...
    if extra_files:
        ...
        try:
            ...
            if staged_files:
                save_external_files_state(
                    ext_state,
                    staging_dir / ".extra_files_state.json",
                )
                extra_files_artifact_ready = True
        except Exception as e:
            print(f"Extra files check failed: {e}")
            errors.append(f"Extra files check failed: {e}")
    ...
    reasons = ["RAN1 document or metadata changed"] if changed else []
    if extra_files:
        # Keep the legacy CLI output for existing RAN1-specific callers/tests.
        reasons.append("external-files-staged" if extra_files_artifact_ready else "external-files-checked")
    return CheckResult(changed=changed, reasons=reasons, errors=errors)


def main() -> None:
    result = check_updates()
    if any(reason.startswith("external-files-") for reason in result.reasons):
        _set_output("extra_files_artifact", str("external-files-staged" in result.reasons).lower())
    _set_output("changed", str(result.changed).lower())
