...
DOWNLOADS_DIR = Path("downloads/ran1")
...
REF_IN_MANUAL_DIR = Path("ref_in_manual/ran1")
...
EXTRA_FILES_STATE_PATH = Path("docs/ran1/.extra_files_state.json")
...
def find_local_latest_schedule(
    ...
    *, meeting_id: str | None = None,
    ...
    schedule_files = [
        ...
        if "schedule" in f.name.lower()
        and (meeting_id is None or _extract_meeting_id(f.name) == meeting_id)
    ]
...
def find_local_vice_chair_schedules(
    ...
    *, meeting_id: str | None = None,
    ...
    for entry in base_dir.iterdir():
        ...
        local = find_local_latest_schedule(entry, meeting_id=meeting_id)
        ...
        if source.local_path is not None:
            ...
            if local.suffix.lower() == ".zip":
                ...
                if extracted is not None:
                    ...
                    raise ValueError(f"No supported document in {local.name}")
        else:
            local = download_schedule_source(source, base_dir)
        if local is None:
            raise RuntimeError(f"Could not download selected schedule: {source.file_info['name']}")
...
def save_schedule_state(
    sources: list[ScheduleSource],
    state_path: Path = Path("docs/ran1/.schedule_state.json"),
    *,
    meeting_id: str | None = None,
...
def get_all_remote_schedule_info(
    ...
    *,
    ...
    ``docs/ran1/.schedule_state.json``. It helps keep the comparison stable
...
def download_external_files(
    ...
    state; when omitted, ``docs/ran1/.extra_files_state.json`` is loaded.
